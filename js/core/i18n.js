    ...
    initTranslations() {
        this.translations = {
            'ru': {
                ...
                'node.rename_hint': 'Двойной клик — переименовать',
                ...
                'history.node_moved': 'Перемещён нод «{{title}}»',
                'history.node_renamed': 'Переименован нод',
                'history.field_changed': 'Изменён параметр',
                'history.connection_created': 'Создана связь',
                'history.connection_removed': 'Удалена связь',
                'history.node_inserted': 'Нод «{{title}}» встроен в связь',
                'history.scheme_loaded': 'Загружена схема',
                'history.scheme_cleared': 'Схема очищена',
                ...
                'io.decrypted_label': 'Расшифрованный текст',
                ...
                'dialog.autosave_found_confirm': 'Найдена автосохранённая схема. Восстановить её?',
                ...
                'settings.compact_mode_desc': 'Уменьшенные ноды и панель — больше места для схемы',
                'settings.grid_desc': 'Показывать точечную сетку на рабочей области',
                ...
                'settings.section_behaviour': 'Поведение',
                ...
                'settings.achievements_list_desc': 'Пасхалки, которые вы уже нашли. Скрытые пока не раскрыты.',
                'settings.achievement_locked': 'Ещё не найдено',
                'settings.achievement_unlocked': 'Получено',
                ...
                'error.compression_decode': 'Ошибка распаковки: данные не являются сжатым LZW-текстом',
                'error.text_too_large': 'Результат ноды «{{node}}» слишком большой: {{length}} символов при лимите {{limit}}. Уменьшите входной текст или уберите часть нодов из цепочки.',
                'error.source_too_large': 'Входной текст слишком большой: {{length}} символов при лимите {{limit}}. Уменьшите его.',
                'error.engine_failed': 'Вычисление прервано: {{message}}',
                'error.engine_cancelled': 'Вычисление отменено.',
                'engine.computing': 'Идут вычисления…',
                'engine.computing_hint': 'Всё в порядке — большой текст или сложная цепочка считаются в фоне, интерфейс продолжает работать. Если ждать долго, уменьшите объём текста.',
                'engine.elapsed': '{{seconds}} с',
                'engine.cancel': 'Отменить',
                'io.truncated': 'Показаны первые {{shown}} из {{total}} символов',
                'io.copy_full': 'Скопировать всё',
                'io.download_full': 'Скачать .txt',
                'notification.result_copied': 'Полный результат скопирован',
                'monitor.truncated': '… ещё {{count}} символов',
                ...
                'help.overview.diagram_input': 'Ввод',
                'help.overview.diagram_cipher': 'Шифр Цезаря',
                'help.overview.diagram_output': 'Вывод',
                ...
                'help.algo.caesar_output': 'Выход: ТУЛЕЗХ (П→Т, Р→У, И→Л, В→Е, Е→З, Т→Х)',
                ...
                'help.algo.morse_mix': 'Русские буквы: ·−−· ·−· …, английские: ... --- ..., пробел между словами — /',
                ...
                'help.algo.morse_cat_dot': '· (точка) → мяу',
                'help.algo.morse_cat_dash': '− (тире) → мрряу',
                'help.algo.morse_cat_space': '/ (пробел между словами) → брряу',
                'help.algo.morse_cat_example': 'Пример кодирования:',
                'help.algo.morse_cat_input': 'Вход: КОТ',
                'help.algo.morse_cat_morse': 'Морзе: −·− −−− −',
                'help.algo.morse_cat_output': 'Кошачий: мрряумяумрряу мрряумрряумрряу мрряу',
                ...
                'help.algo.morse_cat_yo_example': 'Ё с поддержкой → мяумяумрряумяумяу',
                ...
                'help.algo.vigenere_text': 'Текст: ПРИВЕТ, Ключ: КОТКОТ',
                'help.algo.vigenere_result': 'Результат: П+К=Ъ, Р+О=Я, И+Т=Ы, В+К=М, Е+О=У, Т+Т=Е → ЪЯЫМУЕ',
                'help.algo.beaufort_mode': 'Режим Бофор (ключ: КОТ):',
                'help.algo.beaufort_formula': 'Формула: (Ключ - Текст) mod m',
                'help.algo.beaufort_text': 'Текст: ПРИВЕТ, Ключ: КОТКОТ',
                'help.algo.beaufort_result': 'Результат: К−П=Ы, О−Р=Ю, Т−И=Й, К−В=И, О−Е=Й, Т−Т=А → ЫЮЙИЙА',
                ...
                'help.algo.complex_substitution.example_process': 'Обработка ключа: кот!# → неалфавитная часть: !#, алфавитная: кот',
                'help.algo.complex_substitution.example_new_alphabet': 'Новый алфавит: !#котабвгдеёжзийлмнпрсуфхцчшщъыьэ (обрезан до 33 символов, ю и я не попали)',
                'help.algo.complex_substitution.example_encrypt': 'Шифрование "бак": б→#, а→!, к→ё → "#!ё"',
                ...
                'help.algo.simple_substitution.example_encrypt': 'Шифрование "attack": a→c, t→t, c→p, k→g → "cttcpg"',
                ...
                'help.algo.route_transposition.example_order': 'Порядок столбцов по алфавиту ключа: Д(1), К(2), О(3)',
                ...
                'help.algo.compression.title': 'Сжатие',
                'help.algo.compression.desc': 'Два алгоритма сжатия: словарный LZW и простой RLE',
                'help.algo.compression.principle': 'LZW (Лемпель–Зив–Велч) читает текст как байты UTF-8 и строит словарь встреченных цепочек: каждое повторение цепочки заменяется одним кодом словаря. Коды упаковываются по битам (ширина растёт с 9 до 16 бит по мере роста словаря) и выдаются в Base64. RLE просто заменяет серии одинаковых символов на "#<число><символ>", а повторяющиеся фрагменты — на "#<число>[фрагмент]".',
                'help.algo.compression.lzw_example_title': 'Пример LZW:',
                'help.algo.compression.lzw_example_input': 'Текст: абракадабра абракадабра абракадабра (35 символов, 65 байт)',
                'help.algo.compression.lzw_example_output': 'Результат: aCwaCxaKAgDQXUGWkGgcFWAghcEg0IgMKgMMgEPi0RgMTaEVgUbA (52 символа)',
                'help.algo.compression.rle_example_title': 'Пример RLE:',
                'help.algo.compression.rle_example_input': 'Текст: ААААААБББВВГГГГГ',
                'help.algo.compression.rle_example_output': 'Результат: #6АБББВВ#5Г (серии короче 4 символов не сжимаются)',
                'help.algo.compression.feature1': '• Не является шифром — это алгоритмы сжатия; результат LZW для коротких текстов длиннее исходного',
                'help.algo.compression.feature2': '• LZW работает с любыми символами, включая эмодзи; RLE — с любым текстом, спецсимволы # [ ] экранируются',
                'help.algo.compression.feature3': '• Оба алгоритма полностью обратимы; галочка "Распаковка" меняет направление',
                ...
                'help.algo.reverse.desc': 'Реверс всего текста, отдельных слов или режим "Змейка" (по строкам)',
                ...
                'help.algo.planet_enchanter.principle': 'Каждая буква заменяется координатами города, название которого начинается на эту букву. Город выбирается детерминированным генератором случайных чисел, поэтому один и тот же текст всегда даёт один и тот же результат. Пробел превращается в пустую строку.',
                'help.algo.planet_enchanter.example_title': 'Пример:',
                'help.algo.planet_enchanter.example_input': 'Вход: МИР',
                'help.algo.planet_enchanter.example_output': 'Выход:\n44.2103, 43.1332 (Минеральные Воды)\n45.7189, 42.8975 (Ипатово)\n57.3706, 61.3921 (Реж)',
                ...
                'help.algo.multi_replace.example_output': 'Выход: "Прив3т к0т" (правило кот→cat уже не срабатывает: к этому моменту "о" заменена на "0")',
                ...
                'help.algo.atbash.principle': 'Каждая буква заменяется на букву, стоящую на симметричной позиции с конца алфавита (А↔Я, Б↔Ю, В↔Э, …). В 33-буквенном русском алфавите средняя буква П переходит сама в себя.',
                'help.algo.atbash.example_title': 'Пример:',
                'help.algo.atbash.example_input': 'Вход: "ПРИВЕТ"',
                'help.algo.atbash.example_output': 'Выход: "ПОЦЭЪМ" (П→П, Р→О, И→Ц, В→Э, Е→Ъ, Т→М)',
                ...
                'help.algo.perfect.title': 'Идеальный шифр',
                'help.algo.perfect.desc': 'Шифр без ключа с лавинным эффектом: любая буква результата зависит от всех букв сообщения',
                'help.algo.perfect.principle': 'Работает только с символами выбранного алфавита, остальные остаются на своих местах. Единственный секрет — длина сообщения и глубина: из них детерминированно выводится вся "случайность". Каждый уровень глубины — четыре шага, и их результат зависит от всех символов сразу.',
                'help.algo.perfect.steps_title': 'Один уровень глубины:',
                'help.algo.perfect.step1': '1. Перемешивание: символы разлетаются по псевдослучайным позициям (зерно ГПСЧ — длина текста и номер уровня)',
                'help.algo.perfect.step2': '2. Прямая волна: C[i] = (P[i] + C[i−1] + K[i]) mod M — каждый символ впитывает предыдущий и ключевой поток',
                'help.algo.perfect.step3': '3. Обратная волна: C[i] = (C[i] + C[i+1]) mod M — изменения распространяются влево до самого начала',
                'help.algo.perfect.step4': '4. Подстановка: каждый символ проходит через перемешанную таблицу замен (S-box)',
                'help.algo.perfect.steps_result': 'После этого изменение одной буквы меняет почти весь результат. Глубина = количество таких уровней.',
                'help.algo.perfect.example_title': 'Пример (алфавит: русский, глубина 1):',
                'help.algo.perfect.example_input': 'Вход: ПРИВЕТ',
                'help.algo.perfect.example_output': 'Выход: ПЯЯОЮз',
                'help.algo.perfect.example_avalanche': 'Лавина: ПРИВЕС → РцЖнзХ (изменилась одна буква — изменился весь результат)',
                'help.algo.perfect.feature1': '• Расшифровка требует тех же настроек: алфавит и глубина',
                'help.algo.perfect.feature2': '• Регистр входит в алфавит, поэтому результат смешивает строчные и заглавные буквы',
                'help.algo.perfect.feature3': '• "Все символы" = печатные ASCII, перевод строки и кириллица; эмодзи и прочее проходят без изменений',

                'help.algo.shark.title': 'Акулий Шифр',
                'help.algo.shark.desc': 'Превращает текст в акульи звуки "a" разной длины',
                'help.algo.shark.principle': 'Превращает текст в "акульи" звуки. Алфавит делится на уровни по 12 букв; буква кодируется ключевым словом уровня (а, шорк, гура) и повторами юнита "а" — их столько, каков номер буквы внутри уровня. Закодированные буквы разделяются двумя пробелами, пробел превращается в bloop.',
                ...
                'help.algo.shark.example_ru_result': 'Результат: "привет мир" → "шорк а а а а  шорк а а а а а  а а а а а а а а а а  а а а  а а а а а а  шорк а а а а а а а  bloop  шорк а  а а а а а а а а а а  шорк а а а а а"',
                ...
                'help.algo.shark.feature2': '• Закодированные буквы разделяются двойным пробелом',
                ...
                'help.algo.uwu.principle': 'Применяет правила "uwu-speak": заменяет р и л на в, добавляет "заикание" в начале слов и милые смайлики. Не имеет обратной операции.',
                ...
                'help.algo.uwu.rule3': '• В конце слов с некоторой вероятностью добавляются смайлики: UwU, OwO, :3 и т.д.',
                'help.algo.uwu.example_title': 'Пример:',
                'help.algo.uwu.example_input': 'Вход: Привет мир',
                'help.algo.uwu.example_output': 'Выход: Пвивет м-мив (заикание и смайлики зависят от текста, но для одного текста результат всегда одинаков)',
                ...
                'help.algo.navi_terminal.example_output': '[1663459200.0] [PID:4337] MEM_WRITE: 0x4C <OK>\n[1663459200.1] [PID:4337] MEM_WRITE: 0x41 <OK>\n[1663459200.2] [PID:4337] MEM_WRITE: 0x49 <OK>\n[1663459200.3] [PID:4337] MEM_WRITE: 0x4E <OK>',
                ...
                'help.dataloss.a1z26.example1': 'Вход: "А1Б" → "112" (не отличить от числа 112 или от "А-12")',
                ...
                'help.dataloss.math.issue': 'Дробные результаты при делении',
                'help.dataloss.math.desc': 'Деление может дать бесконечную дробь: при обратной операции точность плавающей запятой иногда не возвращает исходное целое число.',
                'help.dataloss.math.example_title': 'Проблемный пример:',
                'help.dataloss.math.example_op': 'Операция: деление на 3',
                'help.dataloss.math.example_result': 'Вход: "10" → "3.3333333333333335" (длинная дробь в тексте)',
                ...
                'help.examples.basic_encryption.scheme_result': 'Результат: "ПРИВЕТ" → "ТУЛЕЗХ"',
                ...
                'help.examples.vigenere.scheme_desc': '[Ввод текста] → вход «Текст» Виженера\n[Секретное слово] → вход «Ключ» Виженера\n[Шифр Виженера] → [Вывод]',
                ...
                'help.examples.fun.scheme_result': 'Результат: "КОТ" → "мрряумяумрряу мрряумрряумрряу мрряу"',
                ...
                'help.hotkeys.cut_mode_desc': 'Режим ножниц: проведите линию через связи, чтобы разорвать их. Удерживайте Alt для временного включения',
                ...
                'help.hotkeys.break_connection_title': 'Shift + клик по точке',
                ...
                'help.hotkeys.pan_title': 'Средняя / правая кнопка мыши',
                'help.hotkeys.pan': 'Панорамирование канваса',
                'help.hotkeys.insert_title': 'Перетащить нод на связь',
                'help.hotkeys.insert': 'Нод встраивается в связь между двумя нодами — не нужно рвать её и соединять заново. Работает и для нодов из палитры',
                'help.hotkeys.palette_tap_title': 'Клик по ноду в палитре',
                'help.hotkeys.palette_tap': 'Добавить нод в центр экрана',
                'help.hotkeys.touch_title': 'Сенсорный экран',
                'help.hotkeys.touch': 'Один палец по пустому месту — панорама, два пальца — масштаб, перетаскивание нодов и портов — как мышью',
                ...
                'option.lzw': 'LZW (словарное, Base64)',
                'option.rle': 'RLE (повторы символов)',
                'option.alphabet_ru': 'Русский (только кириллица)',
                'option.alphabet_en': 'Английский (только латиница)',
                'option.alphabet_all': 'Все символы (ASCII + кириллица)',
                ...
                'easter_eggs.cute_mode.video_unavailable': 'YouTube не показывает видео, когда страница открыта с диска (ошибка 153: сайт без адреса). На GitHub Pages всё играет.',
                'easter_eggs.cute_mode.video_open': 'Открыть на YouTube',
            ...
            'en': {
                ...
                'node.rle_compression': 'Compression',
                ...
                'history.group_moved': 'Moved {{count}} nodes',
                'history.node_moved': 'Moved node "{{title}}"',
                'history.node_renamed': 'Renamed node',
                'history.field_changed': 'Changed a parameter',
                'history.connection_created': 'Connection created',
                'history.connection_removed': 'Connection removed',
                'history.node_inserted': 'Node "{{title}}" inserted into a connection',
                'history.scheme_loaded': 'Scheme loaded',
                'history.scheme_cleared': 'Scheme cleared',
                ...
                'dialog.autosave_found_confirm': 'An autosaved scheme was found. Restore it?',
                ...
                'settings.compact_mode_desc': 'Smaller nodes and panel — more room for the scheme',
                'settings.grid_desc': 'Show the dotted grid on the workspace',
                ...
                'settings.section_behaviour': 'Behaviour',
                ...
                'settings.achievements_list_desc': 'Easter eggs you have already found. Hidden ones stay secret.',
                'settings.achievement_locked': 'Not found yet',
                'settings.achievement_unlocked': 'Unlocked',
                'button.show': 'Show',
                'button.reset': 'Reset',
                ...
                'error.compression_decode': 'Decompression error: the data is not LZW-compressed text',
                'error.text_too_large': 'The result of "{{node}}" is too large: {{length}} characters, the limit is {{limit}}. Reduce the input text or remove some nodes from the chain.',
                'error.source_too_large': 'The input text is too large: {{length}} characters, the limit is {{limit}}. Reduce it.',
                'error.engine_failed': 'Computation aborted: {{message}}',
                'error.engine_cancelled': 'Computation cancelled.',
                'engine.computing': 'Computing…',
                'engine.computing_hint': 'Everything is fine — a large text or a complex chain is being processed in the background and the interface keeps working. If it takes too long, reduce the amount of text.',
                'engine.elapsed': '{{seconds}} s',
                'engine.cancel': 'Cancel',
                'io.truncated': 'Showing the first {{shown}} of {{total}} characters',
                'io.copy_full': 'Copy all',
                'io.download_full': 'Download .txt',
                'notification.result_copied': 'Full result copied',
                'monitor.truncated': '… {{count}} more characters',
                ...
                'help.overview.diagram_input': 'Input',
                'help.overview.diagram_cipher': 'Caesar cipher',
                'help.overview.diagram_output': 'Output',
                ...
                'help.algo.morse_mix': 'Russian letters: ·−−· ·−· …, English: ... --- ..., word gap is /',
                ...
                'help.algo.morse_cat_dot': '· (dot) → мяу',
                'help.algo.morse_cat_dash': '− (dash) → мрряу',
                'help.algo.morse_cat_space': '/ (space between words) → брряу',
                'help.algo.morse_cat_example': 'Encoding example:',
                'help.algo.morse_cat_input': 'Input: CAT',
                'help.algo.morse_cat_morse': 'Morse: -.-. .- -',
                'help.algo.morse_cat_output': 'Cat: мрряумяумрряу мрряумрряумрряу мрряу',
                ...
                'help.algo.morse_cat_yo_example': 'Ё with support → мяумяумрряумяумяу',
                ...
                'help.algo.vigenere_result': 'Result: H+K=R, E+E=I, L+Y=J, L+K=V, O+E=S → RIJVS',
                ...
                'help.algo.beaufort_result': 'Result: K−H=D, E−E=A, Y−L=N, K−L=Z, E−O=Q → DANZQ',
                ...
                'help.algo.complex_substitution.example_process': 'Key processing: cat!# → non-alphabetic part: !#, alphabetic: cat',
                'help.algo.complex_substitution.example_new_alphabet': 'New alphabet: !#catbdefghijklmnopqrsuvwx (cut to 26 symbols, y and z drop out)',
                'help.algo.complex_substitution.example_encrypt': 'Encrypting "bag": b→#, a→!, g→d → "#!d"',
                ...
                'help.algo.simple_substitution.example_encrypt': 'Encrypting "attack": a→c, t→t, c→p, k→g → "cttcpg"',
                ...
                'help.algo.route_transposition.example_order': 'Column order by key letters: E(1), K(2), Y(3)',
                ...
                'help.algo.compression.title': 'Compression',
                'help.algo.compression.desc': 'Two compression algorithms: dictionary-based LZW and simple RLE',
                'help.algo.compression.principle': 'LZW (Lempel–Ziv–Welch) reads the text as UTF-8 bytes and builds a dictionary of sequences it has seen: every repeated sequence is replaced by a single dictionary code. Codes are bit-packed (the width grows from 9 to 16 bits as the dictionary grows) and emitted as Base64. RLE simply turns runs of identical characters into "#<count><char>" and repeated fragments into "#<count>[fragment]".',
                'help.algo.compression.lzw_example_title': 'LZW example:',
                'help.algo.compression.lzw_example_input': 'Text: абракадабра абракадабра абракадабра (35 characters, 65 bytes)',
                'help.algo.compression.lzw_example_output': 'Result: aCwaCxaKAgDQXUGWkGgcFWAghcEg0IgMKgMMgEPi0RgMTaEVgUbA (52 characters)',
                'help.algo.compression.rle_example_title': 'RLE example:',
                'help.algo.compression.rle_example_input': 'Text: AAAAAABBBCCDDDDD',
                'help.algo.compression.rle_example_output': 'Result: #6ABBBCC#5D (runs shorter than 4 are left as they are)',
                'help.algo.compression.feature1': '• Not a cipher — these are compression algorithms; LZW output for short texts is longer than the input',
                'help.algo.compression.feature2': '• LZW works with any characters, including emoji; RLE works with any text, the special characters # [ ] are escaped',
                'help.algo.compression.feature3': '• Both algorithms are fully reversible; the "Decompression" checkbox flips the direction',
                ...
                'help.algo.reverse.desc': 'Reverse the entire text, individual words, or "Snake" mode (by lines)',
                ...
                'help.algo.planet_enchanter.principle': 'Every letter is replaced with the coordinates of a city whose name starts with that letter. The city is picked by a deterministic random generator, so the same text always produces the same result. A space becomes an empty line.',
                'help.algo.planet_enchanter.example_title': 'Example:',
                'help.algo.planet_enchanter.example_input': 'Input: MAP',
                'help.algo.planet_enchanter.example_output': 'Output:\n45.4642, 9.1900 (Milan)\n33.7490, -84.3880 (Atlanta)\n41.1579, -8.6291 (Porto)',
                ...
                'help.algo.multi_replace.example_output': 'Output: "H3ll0 c@t" (the cat→dog rule no longer matches: "a" has already become "@")',
                ...
                'help.algo.perfect.title': 'Perfect Cipher',
                'help.algo.perfect.desc': 'A keyless cipher with an avalanche effect: every output letter depends on every letter of the message',
                'help.algo.perfect.principle': 'Only characters of the chosen alphabet are touched; everything else stays in place. The only secret is the message length and the depth: all the "randomness" is derived from them deterministically. Every depth level is four steps whose result depends on all characters at once.',
                'help.algo.perfect.steps_title': 'One depth level:',
                'help.algo.perfect.step1': '1. Shuffle: characters fly to pseudo-random positions (PRNG seed — text length and level number)',
                'help.algo.perfect.step2': '2. Forward wave: C[i] = (P[i] + C[i−1] + K[i]) mod M — each character absorbs the previous one and a key stream',
                'help.algo.perfect.step3': '3. Backward wave: C[i] = (C[i] + C[i+1]) mod M — changes propagate left all the way to the start',
                'help.algo.perfect.step4': '4. Substitution: every character goes through a shuffled lookup table (S-box)',
                'help.algo.perfect.steps_result': 'After this, changing one letter changes almost the whole result. Depth = the number of such levels.',
                'help.algo.perfect.example_title': 'Example (alphabet: English, depth 1):',
                'help.algo.perfect.example_input': 'Input: HELLO',
                'help.algo.perfect.example_output': 'Output: fpsOq',
                'help.algo.perfect.example_avalanche': 'Avalanche: HELLP → mbJla (one letter changed — the whole result changed)',
                'help.algo.perfect.feature1': '• Decryption needs the same settings: alphabet and depth',
                'help.algo.perfect.feature2': '• Case is part of the alphabet, so the result mixes lower- and upper-case letters',
                'help.algo.perfect.feature3': '• "All symbols" = printable ASCII, newline and Cyrillic; emoji and the rest pass through unchanged',

                'help.algo.shark.title': 'Shark Cipher',
                'help.algo.shark.desc': 'Turns text into shark sounds "a" of varying lengths',
                'help.algo.shark.principle': 'Turns text into "shark" sounds. The alphabet is split into tiers of 12 letters; a letter is encoded as the tier keyword (a, shork, gura) followed by as many "a" units as its index inside the tier. Encoded letters are separated by two spaces; a space becomes bloop.',
                ...
                'help.algo.shark.example_ru_result': 'Result: "привет мир" → "шорк а а а а  шорк а а а а а  а а а а а а а а а а  а а а  а а а а а а  шорк а а а а а а а  bloop  шорк а  а а а а а а а а а а  шорк а а а а а"',
                ...
                'help.algo.shark.feature2': '• Encoded letters are separated by a double space',
                ...
                'help.algo.uwu.principle': 'Applies "uwu-speak" rules: replaces р/л with в, adds stuttering at the start of words and cute faces. Has no reverse operation.',
                ...
                'help.algo.uwu.rule3': '• Cute faces are sometimes appended to words: UwU, OwO, :3 and so on',
                'help.algo.uwu.example_title': 'Example:',
                'help.algo.uwu.example_input': 'Input: Hello world',
                'help.algo.uwu.example_output': 'Output: H-hello world (stutter and faces depend on the text, but the same text always gives the same result)',
                ...
                'help.dataloss.a1z26.example1': 'Input: "A1B" → "112" (indistinguishable from the number 112 or from "A-12")',
                ...
                'help.dataloss.math.issue': 'Fractional results from division',
                'help.dataloss.math.desc': 'Division can produce a repeating fraction; floating point precision sometimes prevents the reverse operation from restoring the original integer.',
                'help.dataloss.math.example_title': 'Problematic Example:',
                'help.dataloss.math.example_op': 'Operation: divide by 3',
                'help.dataloss.math.example_result': 'Input: "10" → "3.3333333333333335" (a long fraction in the text)',
                ...
                'help.examples.vigenere.scheme_desc': '[Text Input] → Vigenère "Text" input\n[Secret Word] → Vigenère "Key" input\n[Vigenère Cipher] → [Output]',
                ...
                'help.examples.fun.scheme_result': 'Result: "КОТ" → "мрряумяумрряу мрряумрряумрряу мрряу" (the cat sounds are Russian)',
                ...
                'help.hotkeys.cut_mode_desc': 'Scissors mode: draw a line across connections to cut them. Hold Alt to enable it temporarily',
                ...
                'help.hotkeys.break_connection_title': 'Shift + click on a port',
                ...
                'help.hotkeys.pan_title': 'Middle / right mouse button',
                'help.hotkeys.pan': 'Canvas panning',
                'help.hotkeys.insert_title': 'Drop a node onto a link',
                'help.hotkeys.insert': 'The node is spliced into the connection between two nodes — no need to cut it and reconnect. Works for palette items too',
                'help.hotkeys.palette_tap_title': 'Click a palette item',
                'help.hotkeys.palette_tap': 'Add the node in the middle of the view',
                'help.hotkeys.touch_title': 'Touch screen',
                'help.hotkeys.touch': 'One finger on empty canvas pans, two fingers zoom; nodes and ports drag like with a mouse',
                ...
                'option.lzw': 'LZW (dictionary, Base64)',
                'option.rle': 'RLE (character runs)',
                'option.alphabet_ru': 'Russian (Cyrillic only)',
                'option.alphabet_en': 'English (Latin only)',
                'option.alphabet_all': 'All symbols (ASCII + Cyrillic)',
                ...
                'easter_eggs.cute_mode.video_unavailable': 'YouTube refuses to play when the page is opened from disk (error 153: no site address). It works on GitHub Pages.',
                'easter_eggs.cute_mode.video_open': 'Open on YouTube',
...
EngineModules.defineClass(I18n);
